...
// ✅ Import aus deiner Datei
import { ETF_INFO, type EtfInfo } from '../data/etfInfo'

// --- Dropdown-Daten ---
// Wichtig: Name/ISIN/TER sollten exakt zu ETF_INFO passen, damit Matching einfach bleibt.
const etfs = ref(
  ETF_INFO.map(e => ({
    id: e.id,
    name: e.name,
...
const selectedEtfRaw = ref('')   // Original aus dem Form (zur Fehlersuche)
const monthlyRate = ref(200)
const years = ref(15)

// --- Helpers fürs Matching ---
function normalizeEtfName(input: unknown): string {
  const name = typeof input === 'string' ? input : ''
  // entfernt z.B. " (TER: 0.07 %)" usw.
  const marker = ' (TER'
...
const selectedEtfInfo = computed<EtfInfo | null>(() => {
  ...
  // 3) enthält Name (falls Raw irgendwas zusätzlich enthält)
...
async function handleSubmitPlan(payload: { etf: string; rate: number; years: number }) {
  ...
  try {
    // In DB speichern mit Name + TER (wie vorher)
...
<template>
  <div class="home">
    ...
    <main class="main">
      ...
      <!-- Debug: zeigt dir sofort, ob überhaupt ein ETF im HomeView ankommt -->
      ...
      <!-- falls kein Match: du siehst dann die gelbe Debug-Zeile oben -->
...
/* ETF-Profil (WICHTIG: Textfarbe, sonst “unsichtbar” auf dunklem Hintergrund) */
.etf-info {
  grid-column: 1 / -1;
  margin-top: 1rem;
  padding: 1rem;
